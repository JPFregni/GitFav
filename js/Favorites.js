...
export class Favorites {
  ...
  load() {
    this.entries =
      JSON.parse(localStorage.getItem('@github-favorites-entry:')) || []
  }

  save() {
    localStorage.setItem(
      '@github-favorites-entry:',
  ...
  async add() {
    ...
    if (userExists)
      return alert('User already exists! Please select a different')

    try {
      const githubUser = await GithubUser.search(nameFavorite)
